...
MIX_JSONL="${MIX_JSONL:-$ROOT_DIR/artifacts/datasets/e4c_numina_cot_mix.jsonl}"
SLICE_JSONL="${SLICE_JSONL:-$ROOT_DIR/artifacts/datasets/e4c_numina_cot_token_matched.jsonl}"
COMMON_EVAL_CONFIG="${COMMON_EVAL_CONFIG:-smol-magpie-ultra}"

REPORT_TO="${REPORT_TO:-none}"
ATTN_IMPLEMENTATION="${ATTN_IMPLEMENTATION:-}"
OUTPUT_DIR="${OUTPUT_DIR:-$ROOT_DIR/outputs/e4c_numina_cot_100k_2gpu}"
...
"$PYTHON_BIN" "$ROOT_DIR/scripts/dataset_tools.py" token-match \
  ...
  --candidate_dataset_config "numina-cot-100k" \
...
CMD=(
  ...
  --run_name "e4c_numina_cot_100k_2gpu"
