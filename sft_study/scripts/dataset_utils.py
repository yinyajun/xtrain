...
def run_mix(args: argparse.Namespace) -> None:
    # 把多个数据源按 --component 配置读取出来，分别做可选过滤/抽样后合并打乱，
    # 最终写成一份统一的 JSONL，适合构造混合 SFT 训练集。
    rows: list[dict[str, Any]] = []
...
def run_token_match(args: argparse.Namespace) -> None:
    # 用 tokenizer 计算参考数据集的总 token budget，再从候选数据集中抽取样本，
    # 直到候选子集的 token budget 追平参考集，适合做“训练 token 数一致”的对照实验。
    ...
    reference_token_lengths = [
        min(
            len(tokenizer.apply_chat_template(row[args.messages_field], tokenize=True, add_generation_prompt=False)),
            args.max_length,
        )
    ...
    for row in candidate_dataset:
        ...
        raw_len = len(tokenizer.apply_chat_template(messages, tokenize=True, add_generation_prompt=False))
...
def run_holdout_split(args: argparse.Namespace) -> None:
    # 从单个数据 split 中随机切出一份固定大小的 eval 集，其余样本作为 train 集，
    # 用于那些只有 train split、但训练时仍想保留本地验证集的数据。
